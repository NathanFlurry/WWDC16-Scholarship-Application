...
enum WWDCSplineEvaluationMethod {
    case Linear, Cubic, CatmullRom, Hermite(CGFloat, CGFloat)
}

extension WWDCSplineEvaluationMethod : Equatable { }

func ==(lhs: WWDCSplineEvaluationMethod, rhs: WWDCSplineEvaluationMethod) -> Bool {
    switch (lhs, rhs) {
...
class WWDCSpline { // See http://paulbourke.net/miscellaneous/interpolation/
    let points: [SCNVector3]
    var method: WWDCSplineEvaluationMethod
    
    init(points: [SCNVector3], method: WWDCSplineEvaluationMethod) {
        self.points = points
        self.method = method
    }
    
    func evalutate(time time: CGFloat) -> SCNVector3 { // Time between 0 and 1
        switch method {
        case .Linear:
...
//            return (a0 * p1) + (a1 * m0) + (a2 * m1) + (a3 * p2)
    ...
    func evaluateRotation(time time: CGFloat, axis: WWDCSplineAxis, samplePrecision precision: CGFloat = 20) -> SCNVector3 {
